...
print(\${variableName})\`;
  ...
  var _MnistPlugin = class _MnistPlugin extends import_sdk.TensorifyPlugin {
    constructor() {
      const definition = {
        // Visual Configuration
        visual: {
          ...
          labels: {
            title: "MNIST Dataset",
            dynamicLabelTemplate: "{train ? 'Train' : 'Test'} \u2022 {download ? 'Download' : 'Local'}",
            showLabels: true,
            labelPosition: "top"
          }
        },
        ...
        outputHandles: [
          {
            ...
            description: "MNIST dataset output"
          }
        ],
        // Settings Configuration
        settingsFields: [
          ...
          },
          {
            ...
            defaultValue: "./data",
            required: true,
            description: "Root directory where dataset will be stored/loaded from"
          },
          {
            key: "train",
            label: "Training Set",
            ...
            defaultValue: true,
            required: false,
            description: "If True, creates dataset from training set, otherwise from test set"
          },
          {
            key: "download",
            label: "Download",
            ...
            defaultValue: true,
            required: false,
            description: "If True, downloads the dataset from internet if not already present"
          },
          {
            key: "transform_input",
            ...
            required: false,
            description: "Variable name for transform (e.g., 'transform'). Leave empty for None"
          },
          {
            ...
            defaultValue: "",
            required: false,
            description: "Variable name for target transform. Leave empty for None"
          }
        ],
        // Plugin Metadata
        emits: {
    ...
    getTranslationCode(settings, children, context) {
      ...
      const variableName = settings.datasetVarName || settings.variableName || "mnist_dataset";
      ...
      const ctor = `torchvision.datasets.MNIST(${paramsStr})`;
      return shouldEmit ? `${variableName} = ${ctor}` : ctor;
    }
  };
  __name(_MnistPlugin, "MnistPlugin");
  var MnistPlugin = _MnistPlugin;
