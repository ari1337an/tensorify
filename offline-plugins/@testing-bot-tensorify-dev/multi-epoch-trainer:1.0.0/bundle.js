...
print(\${variableName})\`;
  ...
  __export(index_exports, {
    default: () => MultiEpochTrainerPlugin
  });
  init_define_process();
  var import_sdk = __toESM(require_dist());
  var _MultiEpochTrainerPlugin = class _MultiEpochTrainerPlugin extends import_sdk.TensorifyPlugin {
    constructor() {
      const definition = {
        // Visual Configuration
        visual: {
        ...
        nodeType: import_sdk.NodeType.TRAINER,
    ...
    getTranslationCode(settings, children, context) {
      ...
      const additionalCode = context ? this.getInput(context, 1) : null;
      const trainModeLine = enableTrainMode ? `    ${modelVariable}.train(True)` : "";
      const printStatement = printEpochLoss ? `    print(f"Epoch {epoch_index + 1}/${numberOfEpochs}, Average Loss: {${avgLossVariable}:.4f}")` : "";
      let additionalCodeBlock = "";
      if (additionalCode?.code) {
        const indentedAdditionalCode = additionalCode.code.split("\n").map((line) => line.trim() ? `    ${line}` : line).join("\n");
...
${printStatement ? printStatement + "\n" : ""}${additionalCodeBlock ? additionalCodeBlock + "\n" : ""}`;
  ...
  __name(_MultiEpochTrainerPlugin, "MultiEpochTrainerPlugin");
  var MultiEpochTrainerPlugin = _MultiEpochTrainerPlugin;
