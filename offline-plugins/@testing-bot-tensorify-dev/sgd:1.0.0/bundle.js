...
print(\${variableName})\`;
  ...
  var _SgdPlugin = class _SgdPlugin extends import_sdk.TensorifyPlugin {
    constructor() {
      const definition = {
        // Visual Configuration
        visual: {
        ...
        settingsFields: [
          ...
          },
          {
            ...
            defaultValue: false,
            required: false,
            description: "Maximize the objective with respect to the params, instead of minimizing"
          }
        ],
        // Plugin Metadata
        emits: {
          variables: [
            {
              ...
              isOnByDefault: true,
              type: import_sdk.NodeType.OPTIMIZER
            }
          ],
          imports: [{ path: "torch.optim", items: ["SGD"] }]
        },
        nodeType: import_sdk.NodeType.OPTIMIZER,
    ...
    getTranslationCode(settings, children, context) {
      ...
      const variableName = settings.optimizerVarName || settings.variableName || "optimizer";
      const shouldEmit = Boolean(settings.emitOptimizerVar);
      const modelParamsInput = settings.model_params_input || "model.parameters()";
      const lr = settings.lr ?? 1e-3;
  ...
  __name(_SgdPlugin, "SgdPlugin");
  var SgdPlugin = _SgdPlugin;
