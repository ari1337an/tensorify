...
Training one epoch function plugin
...
```typescript
import train-one-epochLinearLayer from './dist/index.js';

// Create a linear layer instance
const linearLayer = new train-one-epochLinearLayer();
...
# train-one-epoch Linear Layer: 784 -> 128 features
train-one-epoch_linear_layer = nn.Linear(784, 128, bias=True)
```
