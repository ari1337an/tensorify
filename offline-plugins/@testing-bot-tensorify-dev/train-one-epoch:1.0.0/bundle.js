...
print(\${variableName})\`;
  ...
  __export(index_exports, {
    default: () => TrainOneEpochPlugin
  });
  init_define_process();
  var import_sdk = __toESM(require_dist());
  var _TrainOneEpochPlugin = class _TrainOneEpochPlugin extends import_sdk.TensorifyPlugin {
    constructor() {
      const definition = {
        // Visual Configuration
        visual: {
        ...
        inputHandles: [
          import_sdk.PrevNodeAsInput,
          {
            ...
            dataType: import_sdk.NodeType.CUSTOM,
        ...
        settingsFields: [
          ...
          },
          {
            ...
            required: true,
            description: "Comma-separated variables to destructure from dataloader (e.g., 'images, labels' or 'X, y')"
          },
          {
            key: "device_variable",
            ...
            defaultValue: "",
            required: false,
            description: "Device variable for tensor movement (e.g., 'device'). Leave empty to skip device movement"
          },
          {
            ...
            required: false,
            description: "Condition for when to report progress (e.g., 'batch_no % 100 == 99')"
          },
          {
            ...
            defaultValue: true,
            required: false,
            description: "Whether to include progress reporting in the training loop"
          },
          {
            key: "return_variable",
        ...
        nodeType: import_sdk.NodeType.FUNCTION,
    ...
    getTranslationCode(settings, children, context) {
      ...
      const predictCode = context ? this.getInput(context, 1) : null;
      const lossCode = context ? this.getInput(context, 2) : null;
      const reportCode = context ? this.getInput(context, 3) : null;
      ...
      const destructureVarList = destructureVars.split(",").map((v) => v.trim());
      let destructureCode = "";
      if (deviceVariable) {
        const deviceMovement = destructureVarList.map((v) => `${v} = ${v}.to(${deviceVariable})`).join("\n        ");
        destructureCode = `        ${deviceMovement}`;
      }
      let reportBlock = "";
      if (enableReporting) {
        const indentedReportCode = finalReportCode.split("\n").map((line) => line.trim() ? `            ${line}` : line).join("\n");
        reportBlock = `        if ${reportCondition}:
${indentedReportCode}
            ${returnVariable} = running_loss / (batch_no + 1)`;
      }
      const indentedPredictCode = finalPredictCode.split("\n").map((line) => line.trim() ? `        ${line}` : line).join("\n");
      const indentedLossCode = finalLossCode.split("\n").map((line) => line.trim() ? `        ${line}` : line).join("\n");
...
${reportBlock}
  ...
  __name(_TrainOneEpochPlugin, "TrainOneEpochPlugin");
  var TrainOneEpochPlugin = _TrainOneEpochPlugin;
