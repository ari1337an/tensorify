...
process.on("unhandledRejection", (reason, promise) => {
  ...
  console.error(
    chalk.gray(
      "If the problem persists, please report this issue to the Tensorify team."
    )
  );
  process.exit(1);
});
...
try {
  ...
  if (err.code !== "commander.version" && err.code !== "commander.help") {
    ...
    if (err.message.includes("Unknown command")) {
      ...
      console.error(
        chalk.gray("  • tensorify login     - Authenticate with Tensorify")
      );
      console.error(
        chalk.gray("  • tensorify validate  - Validate plugin structure")
      );
